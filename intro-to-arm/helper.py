...
class EmbryoARMLineEquation(EmbryoARMBase):
    ...
    def print_level_text(self):
        print(
                "Arithmatic instructions take three arguments\n"
                "Example:\n"
                "\t add x0, x1, x2\n\n"
...
class EmbryoARMModulo(EmbryoARMBase):
    ...
    def trace(self):
        if  len(self.asm) != 8:
            print(f"Maximum of 2 instruction aarch64 instructions!  {len(self.asm)} bytes received, but expecting 8!")
            return False
        try:
            self.emu.reg_write(UC_ARM64_REG_X0, self.val_x0)
            self.emu.reg_write(UC_ARM64_REG_X1, self.val_x1)
...
class EmbryoARMBitShift(EmbryoARMBase):
    ...
    def print_level_text(self):
        print(
            ...
            "\tlsl reg1, reg2       <=>     Shift reg1 left by the amount in reg2\n"
            "\tlsr reg1, reg2       <=>     Shift reg1 right by the amount in reg2\n\n"
...
class EmbryoARMPopPush(EmbryoARMBase):
    ...
    def __init__(self, asm=None, should_debug=False):
        super().__init__(asm)
        self.val_rdi = random.randint(10, 100000)
        self.val_stk = random.randint(1000000, 1000000000)
    ...
    def print_level_text(self):
        print(
            ...
            "\tldr x1, [sp], #16\n\n"
            "This loads the value located at the stack pointer into register x1 and then subtracts 16 from the stack.\n\n"
        ...
        print(
            ...
            f"(stack) [{hex(self.BASE_STACK + 0x200000 - 0x8)}] = {hex(self.val_stk)}\n\n"
        )

    def trace(self):
        try:
            self.emu.mem_write(
                self.BASE_STACK + 0x200000 - 0x8, struct.pack("<Q", self.val_stk)
            )
            self.emu.reg_write(UC_X86_REG_RSP, self.BASE_STACK + 0x200000 - 0x8)
        ...
        target = bytearray(struct.pack("<Q", self.val_stk - self.val_rdi))
        return target == self.emu.mem_read(self.BASE_STACK + 0x200000 - 0x8, 8)
...
class EmbryoARMRegSwap(EmbryoARMBase):
    ...
    def print_level_text(self):
        ...
        print(
            "We will now set the following in preparation for your code:\n"
            f"rdi = {hex(self.val_x0)}\n"
            f"rsi = {hex(self.val_x1)}\n\n"
        )
...
class EmbryoARMFib(EmbryoARMBase):
    ...
    def print_level_text(self):
        print(
            ...
            "Registers x19 - x28 are caller saved.\n"
    ...
    def unit_test_user_code(self, fib_arg):
        ...
        self.harness = bytes(ks.asm(f"""
                            mov x3, #{hex(self.code_load_addr & 0xFFFFFFFF)}
                            movk x3, #{hex((self.code_load_addr >> 32) & 0xFFFFFFFF)}, lsl 32
                            blr x3
                            """)[0])
